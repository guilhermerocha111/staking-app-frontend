...
export const useVestingPanel = () => {
  ...
  useMemo(async () => {
    ...
    let d:Date = new Date(Math.ceil(stakes01[0].timestamp.toNumber() * 1000));
    ...
    let stakes1 = stakes01.map((s, i) => {
      ...
      return {
        ...
        stakeFor: s.stakeFor.toNumber(),
        unlocksIn: diffDays(s.stakeUntill.toNumber()),
        timestamp: `${date.toLocaleString('en-GB', { timeZone: 'UTC' })}`,
        percentage: timePercentage(s.timestamp.toNumber(),s.stakeUntill.toNumber())
      };
    });
    ...
    let formatLocks = locks.map((s, i) => {
      ...
      return {
        ...
        timestamp: `${date.toLocaleString('en-GB', { timeZone: 'UTC' })}`,
        percentage: timePercentage(s.vestingTime.toNumber(),s.vestingDuration.toNumber())
      };
    });
