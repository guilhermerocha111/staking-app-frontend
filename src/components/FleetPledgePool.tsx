...
export default function FleetPledgePool({MIN_STAKE_AMOUNT = 5000, MAX_STAKE_AMOUNT = 100000, pool_address = '', pool_label = '', pool_key = ''}) {
  ...
  return (
    <Card className="flex-1">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4">
            ...
            >
              ...
              {isApproved ? (
                <>
                  ...
                  <div className="flex gap-4">
                    <Button type="submit" className="gradient-1 button-3 mt-2 cursor-pointer">Pledge <HiOutlineExternalLink /></Button>
                    <Button type="submit" className="gradient-1 button-3 mt-2 cursor-pointer">Unpledge <HiOutlineExternalLink /></Button>
                  </div>
                  <p className="flex items-center text-sm mt-2">
                    <FiInfo className=" mr-2" /> <span>If you Pledge for the first time the first transaction is only to approve your SMCW, after that you can start pledging</span>
                  </p>
                </>
              ) : (
                <Button
              ...
              {!isApproved && (
                <p className="flex items-center text-sm mt-4">
                  <FiInfo className="text-design-darkBlue2 mr-2" /> If you Pledge
                  for the first time the first transaction is only to approve your
                  SMCW, after that you can start pledging
                </p>
              )}
            </form>
          ) : (
        ...
        <div className={`bgTransparent card-3 grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-1 ${isApproved ? '' : 'blur pointer-events-none select-none'}`}>
          ...
          <div className="grid grid-cols-1 gap-1.5 lg:gap-3">
            ...
            <div className="flex items-center">
              <img
                ...
                />
              <p className="flex items-center gap-2">{addCommasToNumber(9000, 2)}</p>
            </div>
          </div>
        </div>
        <div className="gradient-orange p-px mt-2">
          <div className="gradient-2 button-3 border border-design-blue estimatedRew">
            Total Pledged in Pledge Pool: <img src="/images/coin.png" alt="" />5,000.00
          </div>
        </div>
        <p className="flex flex-col text-md mt-2 bgTransparent" style={{padding: '1rem'}}>
          <div className="flex flex-col text-sm">
            <p>Remember to pledge with the same wallet address that you have linked ingame.</p> <br/>
            <p>Pledging Tier will update at XX:00 UTC, remember to update your Olympus Syndicate Membership Pledge to the required minimum(if needed)before the next weekly snapshot or you won't be able to participate in the next week OSM leaderboard.</p>
            <br/>
            <p className="italic">(*) Minimum pledge needed to activate OSM - Olympus Syndicate Membership NFT</p>
          </div>
        </p>
      </div>
    </Card>
